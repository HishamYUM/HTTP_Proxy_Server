...
int validInput(char* request, char** host, char** path){
	...
	*path = (char*)malloc(sizeof(char)*strlen(line));
	...
	printf("method is %s\npath is: %s\n protocol is: %s", method,*path, protocol);
	...
	while(line != NULL)
	{
		*host = (char*)malloc(sizeof(char) * strlen(line));
		...
		{
			foundHost = TRUE;
			printf("host is %s or %s", *host, hostTitle);
			break;
		}
...
char* constructErrorMsg(int errorNum){
	char* result = (char*)malloc(sizeof(char)*512);								//Initialize the result for empty string		
	char* date = getTime(0, 0, 0);
	if(date == NULL)
		return NULL;
	sprintf(result, 	"HTTP/1.0 %s"				//errorMsgsHead[errorNum]
						"Content-Type: text/html\r\n"
						"Content-Length: %ld\r\n"	//strlen(errorMsgsBody[errorNum])
						"Connection: close\r\n"
						"\r\n"
						"%s"						//Msg body
						,errorMsgsHead[errorNum], strlen(errorMsgsBody[errorNum]), errorMsgsBody[errorNum]);
	free(date);	
	return result;
}
